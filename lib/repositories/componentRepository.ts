 ...
 */
export async function publishComponents(componentIds: string[], projectId?: string | null): Promise<{ count: number; changedComponentIds: string[] }> {
  ...
  let publishedHashQuery = client
    .from('components')
    .select('id, content_hash')
    .in('id', draftComponents.map(d => d.id))
    .eq('is_published', true);
  publishedHashQuery = (await applyProjectScopeToQuery(publishedHashQuery, client, 'components', projectId)).query;
  const { data: publishedComponents } = await publishedHashQuery;
