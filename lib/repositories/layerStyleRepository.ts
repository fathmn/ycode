 ...
 */
export async function publishLayerStyles(styleIds: string[], projectId?: string | null): Promise<{ count: number; changedStyleIds: string[] }> {
  ...
  let draftFetchQuery = client
    .from('layer_styles')
    .select('*')
    .in('id', styleIds)
    .eq('is_published', false)
    .is('deleted_at', null);
  draftFetchQuery = (await applyProjectScopeToQuery(draftFetchQuery, client, 'layer_styles', projectId)).query;
  const { data: draftStyles, error: fetchError } = await draftFetchQuery;
...
export async function getUnpublishedLayerStyles(projectId?: string | null): Promise<LayerStyle[]> {
  ...
  let draftListQuery = client
    .from('layer_styles')
    .select('*')
    .eq('is_published', false)
    .is('deleted_at', null)
    .order('created_at', { ascending: false });
  draftListQuery = (await applyProjectScopeToQuery(draftListQuery, client, 'layer_styles', projectId)).query;
  const { data: draftStyles, error } = await draftListQuery;
...
export async function hardDeleteSoftDeletedLayerStyles(projectId?: string | null): Promise<{ count: number }> {
  ...
  scopedQuery = (await applyProjectScopeToQuery(scopedQuery, client, 'layer_styles', projectId)).query;
  const { data: deletedDrafts, error } = await scopedQuery;
 ...
 */
export async function getUnpublishedLayerStylesCount(projectId?: string | null): Promise<number> {
