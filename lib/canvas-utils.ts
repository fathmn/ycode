...
const CANVAS_CSS_VERSION = '0.2.2.0';
...
// Studio: editor viewport overrides for imported inline styles intentionally
// support raw viewport lengths only, e.g. `100svh`, `80vh`, `100dvh`.
// Composite expressions such as `calc(100svh - 72px)` must be normalized by
// the importer before annotation, otherwise the editor leaves them
// source-truthful instead of guessing.
const RAW_VIEWPORT_LENGTH_PATTERN = new RegExp(
...
function viewportLengthToPixels(value: string | null, referenceHeight: number): number | null {
  ...
  const match = value.trim().match(RAW_VIEWPORT_LENGTH_PATTERN);
