...
import { triggerStudioProductionDeployment, writeStudioAuditLogForContext } from '@/lib/studio-platform';
import type { VerifiedStudioPublishContext } from '@/lib/studio-platform';
...
async function warmStudioRoutes(routes: string[]): Promise<{ warmed: number; total: number } | null> {
  if (process.env.VERCEL !== '1' || routes.length === 0) return null;
  const configuredHost = (
    process.env.STUDIO_APP_HOST
  ...
  const absoluteHost = /^https?:\/\//i.test(configuredHost)
    ? configuredHost
    : `https://${configuredHost}`;
  let origin: URL;
  try {
    origin = new URL(absoluteHost);
  } catch {
    return null;
  }
  ...
  return warmRoutes(routes, request);
}

export async function executeStudioPublish(input: {
  context: VerifiedStudioPublishContext;
    ...
    if ((collectionIds && collectionIds.length > 0) || (collectionItemIds && collectionItemIds.length > 0)) {
      ...
      if (collectionIds && collectionIds.length > 0) {
        for (const collectionId of collectionIds) {
          const items = await getAllItemsByCollectionId(collectionId, false, false, projectId);
      ...
      const publishedCollections = await getCollectionsRaw(true, projectId);
      const publishedCollectionById = new Map(publishedCollections.map(c => [c.id, c]));
      const draftFieldsByCollection = groupByCollectionId(await getAllFields(false, projectId));
      const publishedFieldsByCollection = groupByCollectionId(await getAllFields(true, projectId));
      const draftItemsByCollection = groupByCollectionId(await getAllItemsRaw(false, projectId));
      const publishedItemsByCollection = groupByCollectionId(await getAllItemsRaw(true, projectId));
      ...
      const collectionsNeedingCleanup = await getCollectionsNeedingDeletionCleanup(projectId);
  ...
  if (changedLayerStyleIds.length > 0) {
    try {
      ...
      if (sync.affectedComponentIds.length > 0) {
        ...
        const repubResult = await publishComponents(sync.affectedComponentIds, projectId);
  ...
  if (isPublishingAll) {
    ...
    try {
      ...
      const pendingDeleteIds = await getSoftDeletedPageIds(projectId);
    ...
    if (publishLocales) {
      try {
        localisationResult = await publishLocalisation(projectId);
  ...
  // Copy draft CSS to published CSS
  {
    ...
    try {
      ...
      try {
        ...
        cssRegenerated = await regenerateDraftCssSafe(context.project.id);
      } catch (error) {
      ...
      if (cssRegenerated) {
        result.changes.css = await publishCSS(context.project.id);
  ...
  try {
    ...
    try {
      ...
      if (currentColorHash !== lastColorHash) {
        ...
        await setSetting('color_variables_published_hash', currentColorHash, projectId);
      }
    } catch (err) {
      globalChanged = true;
    ...
    const warmResult = await warmStudioRoutes(liveRoutesToWarm);
    if (warmResult) {
      console.log(
  ...
  try {
    result.published_at_setting = await savePublishedAt(publishedAt, context.project.id);
  } catch {
    // Silently handle - non-fatal
  }
  ...
  await writeStudioAuditLogForContext({
    ...
    metadata: {
      ...
      draftHash: context.publishVerification.draftHash,
      customCode: context.publishVerification.customCode,
