...
export function registerStyleTools(server: McpServer, projectContext: McpProjectContext = {}) {
  server.tool(
    ...
    {},
    async () => {
      const styles = await getAllStyles(false, projectContext.projectId);
  ...
  server.tool(
    ...
    async ({ page_id, layer_id, style_id }) => {
      ...
      const style = (await getAllStyles(false, projectContext.projectId)).find((s) => s.id === style_id);
      if (!style) {
...
- Detach all styles (keep the current look as local classes): style_ids: []`,
    ...
    async ({ page_id, layer_id, style_ids }) => {
      ...
      const stylesById = new Map((await getAllStyles(false, projectContext.projectId)).map((s) => [s.id, s]));
