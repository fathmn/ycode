...
export const usePagesStore = create<PagesStore>((set, get) => ({
  ...
  publishPage: async (pageId) => {
    set({ isLoading: true, error: null });
    try {
      const response = await studioFetch('/ycode/api/publish', {
        method: 'POST',
  ...
  duplicatePage: async (pageId) => {
    ...
    (async () => {
      try {
        const response = await studioFetch(`/ycode/api/pages/${pageId}/duplicate`, {
          method: 'POST',
        });
        ...
        try {
          const [pagesResponse, foldersResponse] = await Promise.all([
            studioFetch('/ycode/api/pages'),
            studioFetch('/ycode/api/folders')
          ]);
  ...
  duplicateFolder: async (folderId) => {
    ...
    (async () => {
      try {
        const response = await studioFetch(`/ycode/api/folders/${folderId}/duplicate`, {
          method: 'POST',
        });
  ...
  refetchPageCollectionItem: async (pageId) => {
    ...
    try {
      ...
      const response = await studioFetch(`/ycode/api/pages/${pageId}/collection-item?itemId=${currentItem.id}`);
      const result = await response.json();
