...
export const useLayerStylesStore = create<LayerStylesStore>((set, get) => ({
  ...
  loadStyles: async () => {
    ...
    try {
      const response = await studioFetch('/ycode/api/layer-styles');
  ...
  createStyles: async (inputs) => {
    ...
    try {
      const response = await studioFetch('/ycode/api/layer-styles/bulk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  updateStyle: async (id, updates) => {
    ...
    try {
      const response = await studioFetch(`/ycode/api/layer-styles/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  restoreLayerStyles: async (styleIds) => {
    ...
    for (const styleId of stylesToRestore) {
      try {
        const response = await studioFetch(`/ycode/api/layer-styles/${styleId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
