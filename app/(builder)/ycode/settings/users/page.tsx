...
export default function UsersSettingsPage() {
  ...
  const fetchUsers = async () => {
    try {
      const response = await studioFetch('/ycode/api/auth/users');
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await studioFetch(`/ycode/api/auth/users?id=${userToDelete.id}`, {
        method: 'DELETE',
      });
      ...
      if (response.ok && result.data?.success) {
        ...
        setInviteError(result.error || 'Nutzer konnte nicht entfernt werden.');
        setTimeout(() => setInviteError(null), 5000);
      }
    } catch (error) {
      console.error('Failed to delete user:', error);
      setInviteError('Nutzer konnte nicht entfernt werden.');
      setTimeout(() => setInviteError(null), 5000);
    } finally {
  ...
  const handleChangeRole = async (userId: string, newRole: string) => {
    try {
      const response = await studioFetch(`/ycode/api/auth/users?id=${userId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok && result.data?.success) {
        ...
        setInviteError(result.error || 'Rolle konnte nicht geändert werden.');
        setTimeout(() => setInviteError(null), 5000);
      }
    } catch (error) {
      console.error('Failed to change role:', error);
      setInviteError('Rolle konnte nicht geändert werden.');
      setTimeout(() => setInviteError(null), 5000);
    }
  };
