...
export default function StaticExportSettings() {
  ...
  const loadSettings = async () => {
    setIsLoading(true)
    try {
      const response = await studioFetch('/ycode/api/apps/static-export/settings')
  ...
  const pollExportStatus = useCallback((jobId: string) => {
    ...
    pollRef.current = setInterval(async () => {
      try {
        const res = await studioFetch('/ycode/api/apps/static-export/status')
  ...
  const handleExport = async () => {
    ...
    try {
      if (hasChanges) {
        const saveRes = await studioFetch('/ycode/api/apps/static-export/settings', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(config),
        })
      ...
      const response = await studioFetch('/ycode/api/apps/static-export/export', {
        method: 'POST',
      })

      if (response.ok) {
        const statusRes = await studioFetch('/ycode/api/apps/static-export/status')
  ...
  const handleDisconnect = async () => {
    try {
      await studioFetch('/ycode/api/apps/static-export/settings', {
        method: 'DELETE',
      })
