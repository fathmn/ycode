...
import { getCallerInfo } from '@/lib/roles-server';
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (role === 'admin' && roleCheck.context.role !== 'studio_admin') {
      return noCache({ error: 'Only a Studio admin can assign the admin role' }, 403);
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (userId === roleCheck.context.actorUserId) {
